...
Code review run on 2026-05-06 against `main`. Three reviewer layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) launched in parallel with no shared context. Diff size: 532 lines, 13 files.

Triage outcome: **2 must-fix patches** (CI clippy gate failures introduced by Story 1.4), **3 should-fix patches** applied inline, **7 deferred** (forward-looking or tracing-subscriber upstream behavior), the rest dismissed as theoretical / forward-looking.

**Patches applied in commit `review(1-4-…): address review feedback`:**

- [x] [Review][Patch] **Clippy `needless_pass_by_value` on `dispatch(cli: Cli)`** [`src/cli.rs:110`] — converted to `dispatch(cli: &Cli)` with destructured `match &cli.command`; updated caller `dispatch(&cli)`. Was breaking `cargo clippy --all-targets -- -D warnings` (the CI gate from Story 1.2). Root cause: original implementation took `cli` by value but only matched on the `command` field with `_` patterns, never moving from it.
- [x] [Review][Patch] **Clippy `unnecessary_wraps` on `render_root_help`** [`src/cli.rs:122`] — function only ever returned `Ok(())`; changed signature to `fn render_root_help()` and updated the `None` arm of `dispatch` to wrap the call in `Ok(())`. Same CI-gate root cause as above. Both clippy errors had been masked because `cargo clippy` was permission-blocked in the dev session (per Debug Log References) and never run locally.
- [x] [Review][Patch] **`from_arg_matches` Err had no stderr diagnostic** [`src/cli.rs:79-86`] — error path mapped to `Error::Config` but never routed through `output::diag`, so the user would see exit code 10 with no message. Added an `output::diag(rendered.trim_end())` call inside the `map_err` closure that mirrors the existing `handle_clap_error` shape. This is a `Cli::from_arg_matches` post-`try_get_matches` path which is essentially unreachable in practice with clap-derive, but the silent-exit landmine is gone.
- [x] [Review][Patch] **`build.rs` rerun-if-changed missed `.git/packed-refs`** [`build.rs:13`] — after `git gc` (or on shallow clones) loose refs are migrated to the single packed-refs file, and the existing `cargo:rerun-if-changed=.git/refs/heads` directive doesn't catch updates there. Added an explicit watch on `.git/packed-refs`. Failure mode this prevents: `LCRC_BUILD_COMMIT` silently goes stale when committing to a packed-refs-using checkout.
- [x] [Review][Patch] **`version_cold_under_200ms` test had inverted-inequality reasoning** [`tests/cli_help_version.rs:65-86`] — the comment claimed warm latency is "necessarily ≤ the cold budget, so this remains a valid upper-bound check," but warm wall time is a *lower bound* on cold wall time, not an upper bound. A passing warm test does not prove the cold AC. Renamed the test to `version_warm_under_200ms`, rewrote the comment to flag the asymmetry honestly (warm > 200 ms ⇒ cold > 200 ms is the only direction it gates), and pointed at T8.5 as the cold ground truth. Also hardened the empty-vec fallback (`unwrap_or(Duration::ZERO)` would have silently passed if a future edit zeroed the sample loop) → `expect("loop runs 3 iterations, samples is non-empty")`.

**Deferred (filed against this story; revisit when the underlying constraint changes):**

- [x] [Review][Defer] Subscriber init failure silently discards non-double-init errors [`src/cli.rs:113`] — deferred. `tracing-subscriber` 0.3's `TryInitError` does not distinguish double-init from "another global subscriber already installed by a linked library" or other registry construction failures. A future patch could match on the underlying source, but the current `tracing-subscriber` API doesn't expose enough; revisit if/when the crate adds a richer error kind.
- [x] [Review][Defer] Tracing subscriber not installed on the clap-error `other` arm [`src/cli.rs:88-103`] — deferred. AC4 says "any non-`--help`/`--version` invocation"; strictly the `other` (usage-error) arm qualifies, but it returns immediately without running any subcommand body, so there are no events to observe. Revisit if Epic 6 wires tracing emits into the CLI parse path.
- [x] [Review][Defer] clap "other" `ErrorKind` includes `Io`/`Format`/`InvalidUtf8` mapped to `ConfigError` [`src/cli.rs:96-101`] — deferred. Broken-pipe writing the help text → exit 10 (semantically wrong; should be a different code or 0 for SIGPIPE). The fix needs a richer mapping table; out of scope for Story 1.4. Pre-existing pattern across `output::*`.
- [x] [Review][Defer] Malformed `RUST_LOG` falls back to `info` silently [`src/util/tracing.rs:39`] — deferred. This is the documented `tracing-subscriber` behavior, and emitting a warning before the subscriber is installed would itself need stderr writes outside the discipline. Revisit if Story 6.x adds a `--log-level` knob.
- [x] [Review][Defer] `clap::error::ErrorKind` is `#[non_exhaustive]`; new variants fall to `ConfigError` [`src/cli.rs:91-101`] — deferred. Forward-compat exposure on clap minor-version updates. Mitigation is a `clap` upgrade-time audit; no current bug.
- [x] [Review][Defer] `build.rs` does not handle git worktrees (`.git` is a regular file pointing into `.git/worktrees/`) [`build.rs:18-31`] — deferred. The `git rev-parse --short HEAD` exec still resolves correctly inside a worktree (git itself reads `.git/worktrees/<name>/HEAD`); the rerun-if-changed paths just won't catch HEAD movement in a worktree. Solo-dev workflows don't typically use worktrees; revisit if the bmad-auto.sh orchestrator starts using them.
- [x] [Review][Defer] Out-of-scope edit to `scripts/bmad-auto.sh` not in the story's File List [`scripts/bmad-auto.sh:133-138`] — deferred. The 8-line change rewires the `tee` invocation to preserve `claude`'s exit code via `PIPESTATUS` (orchestration improvement, not a Story 1.4 deliverable). Harmless but technically outside the file list. Leave in this commit to avoid a separate orchestration PR.

**Dismissed (16 findings; noise / theoretical / handled elsewhere):** trailing-newline reshaping by `output::result`; brittle stderr `error:` literal in `unknown_subcommand_exits_config_error`; possible help/no-args template divergence in a hypothetical future edit; `Cli::command()` called twice on the no-args path (cheap); `OnceLock`-wrapped `render_long` panic in a hypothetical Story 6.6 edit; non-existent paths in `cargo:rerun-if-changed` (cargo treats as always-stale, sub-millisecond cost); `is_terminal()` evaluated at init time (CLI process, not a daemon); concurrent `init()` race (single-threaded `dispatch`); reproducible-builds `SOURCE_DATE_EPOCH` (out-of-scope for v0.x); `version_warm_under_200ms` flaky on slow CI runners (AC pinned to reference rig); SIGPIPE panic on `lcrc --help | head -1` (pre-existing across the `output` module; defer to that module's owner story); BUILD_COMMIT internal-newline (`git rev-parse --short HEAD` cannot return that); cargo-directive parse failure on internal newlines (same); non-UTF-8 args (clap default behavior, acceptable); `?`-vs-`map_err` wording mismatch in spec T4.5 (semantically equivalent); spec internal `cmd::` vs `cli::` namespace contradiction (spec wording bug; code is correct); resolved-decision wording about no-args path tracing-init cost (no AC violated).

**Post-patch CI mirror:** `cargo fmt --all -- --check` clean, `cargo clippy --all-targets --all-features -- -D warnings` clean, `cargo test --all-targets --all-features` 31 passed (4 suites, 0.47 s).
...
**AC5 (NFR-P7 cold latency):** `tests/cli_help_version.rs::version_warm_under_200ms` (renamed in code review — see Review Findings) runs `lcrc --version` three times and asserts `min < 200 ms`. The AC measures *cold* (page-cache cleared) latency; tests run after `cargo build` so the binary's pages are warm. Warm wall time is a *lower bound* on cold wall time, not an upper bound: a warm pass does **not** prove the cold AC, but a warm regression (warm > 200 ms) **does** prove cold > 200 ms. The test gates the easy direction; T8.5 manual measurement is the cold ground truth (permission-blocked in this dev session — recorded by the operator after merge). The hot path stays cheap by design: clap parses `--version`, `parse_and_dispatch` short-circuits on `DisplayVersion` before `dispatch` is called, so the tracing subscriber is never installed on the version path.
